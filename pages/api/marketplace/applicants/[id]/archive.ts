...
export default async function handler(req: NextApiRequest, res: NextApiResponse){
  ...
  try{
    ...
    try{ const applicantTarget = items[idx].applicantId || items[idx].applicantName || items[idx].id; addNotification(applicantTarget, 'archived', { appId: items[idx].id, itemId: items[idx].itemId }) }catch(e){}
    fs.writeFileSync(FILE, JSON.stringify(items, null, 2))
    return res.status(200).json({ application: items[idx] })
  }catch(err){ console.error('marketplace archive api error', err); return res.status(500).json({ error: 'failed' }) }
}
