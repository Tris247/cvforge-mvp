...
describe('legacy analytics conversion', () => {
  ...
  it('converts legacy data/analytics.json array to JSONL', () => {
    ...
    // run the lib in a child Node process with cwd set to tmp so the conversion runs there
    const absLib = path.resolve(process.cwd(), 'lib', 'analytics.js')
    execSync(`node -e "require('${absLib.replace(/\\/g,'\\\\')}')"`, { cwd: tmp })
